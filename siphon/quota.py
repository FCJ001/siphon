    ...
    for acct in accounts.all_accounts():
        ...
        if windows is None:
            ...
            if reason.startswith("usage 端点返回 401"):
                accounts.mark_account_open(acct.id, time.time() + 3600)
                await _notify(f"账号 {acct.alias or acct.key_tail} 的 key 鉴权失败(401), 已停用")
        ...
        accounts.headroom_set(acct.id, worst, resets)
        db.execute(
